...
class Quiz:
    def __init__(self, master, operator, difficulty, add_scores, sub_scores, mult_scores, div_scores, final_scores, data_list, score_list):
        ...
        self.operator = operator #store variable in case back button is pressed or test is repeated
        self.difficulty = difficulty #store variable in case test is repeated
        self.old_score = score_list[int(difficulty)-1]
        self.NUMBER_OF_QUESTIONS = 10 #how many questions should there be in each test
    ...
    def repeat(self):
        self.canvas.destroy()
        Quiz(self.master, self.operator, self.difficulty)
...
def read_file():
    ...
    #this will make it easier for me to work out which value corresponds to what instead of listing the values from 0 to 15
    return data
